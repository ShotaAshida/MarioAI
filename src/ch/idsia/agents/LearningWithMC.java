...
public class LearningWithMC implements LearningAgent{
	...
	public void learn(){
		for(int i = 0; i < numOfTrial; ++i){
			...
			if(run() >= 4096.0f){
				for( int j = 0 ; j < MCAgent.best.size(); j++ ){
					System.out.print(MCAgent.best.get(j)+ " ");
				}
				show();
			}
			if( i % 1000 == 999 &&  MCAgent.bestScore > 2107 )
				show();
		}
	...
	public float run(){
		...
		if(reward > MCAgent.bestScore){
			...
			for(int i = 0 ; i < MCAgent.best.size(); i++){
				if( i < MCAgent.best.size()-1 ){
					System.out.print( MCAgent.best.get(i) + " " );
				}else{
					System.out.println( MCAgent.best.get(i) );
				}
			}
		}
