...
public class MCAgent extends BasicMarioAIAgent implements Agent{
	...
	public boolean[] getAction(){
		...
		if(!mode){
			if( distancePassedPhys > 1323 && distancePassedPhys < 2510 ){
				int y = marioEgoCol;
				int x = marioEgoRow;
				...
				if( distancePassedPhys == 1899 ){
					...
					action[Mario.KEY_RIGHT] = false;
				}else if( distancePassedPhys == 1568 ){
					action[Mario.KEY_JUMP] = isMarioAbleToJump || !isMarioOnGround ;
				}
				
				if( getReceptiveFieldCellValue( x-2, y+1 )  == GeneralizerLevelScene.BRICK ){
					action[Mario.KEY_JUMP] = true;
				}
				
				if( distancePassedPhys > 1323 && distancePassedPhys < 1370 && ST < 40 ){
					action[Mario.KEY_RIGHT] = false;
					ST++;
					//System.out.println("ストップ");
				}
				
				if( distancePassedPhys >= 1370 ){
					ST = 0;
				}
			...
			if( distancePassedPhys > 1323 && distancePassedPhys < 2510 ){
				...
				if( distancePassedPhys == 1899 ){
					...
					System.out.println("ブロック届けーーーーーーーーーーーー");
					action[Mario.KEY_JUMP] = isMarioAbleToJump || !isMarioOnGround ;
				}
